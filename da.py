...
if __name__ == "__main__":
    ...
    for target_dir in dirs:
        if machine_type == 'todos':
            target_dir = None
        model_path = os.path.join(params.model_dir, machine_type, 'model_{}.pth'.format(machine_type))
        ...
        #     n_try += 1
        #     bgm = BayesianGaussianMixture(n_components=min(30, 4 * model.z_dim), # PONER A 7
        ...
        #                                   covariance_type='full').fit(mu_train)
        ...
        mu = torch.zeros((n_windows, model.z_dim)).to(device)   # mu=0 para generación desde el centroide
        std = torch.ones((n_windows, model.z_dim)).to(device)  # std=1 para mu=0
