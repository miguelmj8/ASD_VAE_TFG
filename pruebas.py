...
import re
import numpy as np
from sklearn.metrics import confusion_matrix, ConfusionMatrixDisplay, roc_auc_score, RocCurveDisplay, roc_curve, average_precision_score, recall_score, fbeta_score, accuracy_score
...
# labels_pred_val = np.column_stack([labels_pred_val,labels_pred_1csvm_val])
...
precision_eval = [average_precision_score(labels_eval,labels_pred_eval[:,i]) for i in range(labels_pred_eval.shape[1])]
precision_eval = np.array(precision_eval)
recall_eval = [recall_score(labels_eval,labels_pred_eval[:,i]) for i in range(labels_pred_eval.shape[1])]
recall_eval = np.array(recall_eval)
roc_eval = [roc_auc_score(labels_eval,as_pred_eval[:,i]) for i in range(as_pred_eval.shape[1])]
roc_eval = np.array(roc_eval)

fpr, tpr, th = roc_curve(labels_eval, as_pred_eval[:, as_type])
idx = np.argmin(np.abs(th - thresholds[as_type]))
...
ax20.plot(
    ...
    color="red",
    markersize=5,
    label=f"Umbral Actual ({thresholds[as_type]:.4f})",
)

# Actualizar la leyenda para incluir el marcador
